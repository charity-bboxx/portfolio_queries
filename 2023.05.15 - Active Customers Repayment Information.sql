with contract_details as (	
	...
	where 
		dcs.date_timestamp::DATE = '20230302' --<--- enter the specific date here
...
total_paid_to_date as (
	...
	where 
		p.state = 'posted'
		and p.transaction_date::DATE <= '20230302'::DATE  --<--- enter the specific date here
	group by 
		p.payg_account_id
		) 
	, 
...
total_paid_six_months as (
	...
	where 
		p.state = 'posted'
		and p.transaction_date::DATE >= '20230302'::DATE - 180 --<--- enter the specific date here
		and p.transaction_date::DATE <= '20230302'::DATE
	group by 
		p.payg_account_id
		)
	, 
...
lifetime_ur  as (
	...
	from kenya.daily_customer_snapshot dcs
	where 
		dcs.date_timestamp::DATE <= '20230302'::DATE  --<--- enter the specific date here
	group by 
		dcs.account_id 
		)
	, 
...
last_six_mo_ur as (
	...
	from kenya.daily_customer_snapshot dcs
	where 
		dcs.date_timestamp::DATE >= '20230302'::DATE - 180 --<--- enter the specific date here
		and dcs.date_timestamp::DATE <= '20230302'::DATE
	group by 
		dcs.account_id 
		)
...
--where 
	--contract_details.unique_customer_id = 'BXCK00166241'