...
FROM
	...
	upper(agent.sales_agent_bboxx_id)  = upper(substring (sales.sales_person,0,7))
...
(
	...
	from last_phon l
		left join src_odoo13_kenya.account_payment p
			on l.payg_account_id = p.payg_account_id
...
active as 
	...
	left join kenya.customer as customer   
		...
		sales_details.unique_account_id = customer.unique_account_id
	     ...
	     left join last_phone as last_phone on 
	     last_phone.payg_account_id = customer.unique_account_id 
	where customer.customer_final_status is null
	--and today.date_timestamp::date = current_date
...
last_30_downpayments as 
	...
	from kenya.rp_retail_sales 
	where 
		downpayment_date::date >= CURRENT_DATE - INTERVAL '1 MONTH'
        group by 1,2
	),
